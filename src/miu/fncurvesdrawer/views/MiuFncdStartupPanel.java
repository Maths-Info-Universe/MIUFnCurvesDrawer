 ...
 */
public class MiuFncdStartupPanel extends JPanel{
    
    private ArrayList<JCheckBox> checkboxes = new ArrayList<JCheckBox>();
    private int step = 1;
    private MIUConfigManager config;
    
    public MiuFncdStartupPanel(MIUConfigManager config){
        this.config = config;
        this.setLayout(null);
        int j = 9;
        for(int i = 0; i <= 3; i++){
    ...
    protected void paintComponent(Graphics g) {
        ...
        ImageIcon icon = new ImageIcon(getClass().getResource("/miu/fncurvesdrawer/resources/images/splash.png"));
        ...
        icon = new ImageIcon(getClass().getResource("/miu/fncurvesdrawer/resources/images/camer.gif"));
        DateTime d = new DateTime(new Date());
        if((d.getDayOfMonth() >= 18 && d.getMonthOfYear() == 12) || (d.getDayOfMonth() <= 10 && d.getMonthOfYear() == 1)){
            icon = new ImageIcon(getClass().getResource("/miu/fncurvesdrawer/resources/images/christmas.gif"));
